...
namespace menu {
	...
	void world_menu::render() {
		renderer::addSubmenu("World", "World", [](core* core) {
			...
			core->addOption(submenuOption("World glow")
			...
			core->addOption(submenuOption("Wave Point Manager")
				.setTarget("Wave Point Manager"));

			core->addOption(submenuOption("Train Manager")
				.setTarget("Train Manager"));
			...
			core->addOption(submenuOption("Black Hole Manger ")
		...
		renderer::addSubmenu("Train Manager ", "Train Manager", [](core* core) {
			core->addOption(toggleOption("train spawner")
				.addClick([=] { m_vars.m_spawn; }));
			});


		renderer::addSubmenu("Wave Point Manger ", "Wave Point Manager", [](core* core) {
	...
	void world_menu::update() {
		...
		if (m_vars.m_spawn) {
			...
			if (m_vars.m_train && ENTITY::DOES_ENTITY_EXIST(m_vars.m_train)) {
				menu::notify::stacked("Spawned Train");
				return;
			}

			menu::control::requestModel(0x3D6AAA9B);
			menu::control::requestModel(0x0AFD22A6);
			menu::control::requestModel(0x36DCFF98);
			menu::control::requestModel(0x0E512E79);
			menu::control::requestModel(0x264D9262);
			menu::control::requestModel(0xD1ABB666);

			m_vars.m_train = VEHICLE::CREATE_MISSION_TRAIN(15, { pos.x, pos.y, pos.z, }, true, true, true);
		}
