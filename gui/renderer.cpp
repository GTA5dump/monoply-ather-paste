...
namespace menu::renderer {
	...
	void renderer::draw_header() {
		...
		render::draw_text(" ", JUSTIFY_CENTER, { m_position.x, render::get_rect_base(m_header.m_height) }, m_header.m_font_scale, m_header.m_font, { 2.1f, 2.1f }, m_header.m_text_color, true);
	...
	void renderer::drawOverlay()
	{
		...
		std::string Format = "monopoly" + std::string(" | FPS: ") + std::to_string(FPS) + std::string(" | ") + (std::string)Buffer;
