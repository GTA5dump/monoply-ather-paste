...
namespace menu {
	bool waitForControl(Entity entity) {
		int timeout = 1000; // You can adjust the timeout value
	...
	inline Vehicle spawn_vehicle(std::uint32_t hash) {
		...
		Vector3 offset = { 0.0, forward, 1.0 }; // Offset in local coordinates
		Vector3 spawnPosition = ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(PLAYER::PLAYER_PED_ID(), offset);
		...
		*(unsigned short*)patterns::set_this_thread_networked = 0x0574; // We restore it so we don't get detected 
		if (!ENTITY::DOES_ENTITY_EXIST(the_vehicle)) {
			return NULL; 
		}
		...
		if (waitForControl(the_vehicle)) {
			// Make the vehicle invisible
			ENTITY::SET_ENTITY_ALPHA(the_vehicle, 0, false);
			...
			// Restore entity alpha to default value if necessary
			ENTITY::SET_ENTITY_ALPHA(the_vehicle, 255, false);
		}
