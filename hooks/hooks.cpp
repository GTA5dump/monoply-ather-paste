...
namespace base::hooks {
	...
	bool patterns() {
		...
		batch.Add({ XOR("GS"), XOR("83 3D ? ? ? ? ? 75 17 8B 43 20 25") }, [](Ptr ptr) {
		...
		batch.Add({ XOR("PRP"), XOR("48 8B 05 ? ? ? ? 0F B7 50 10 48 8B 05") }, [](Ptr ptr) {
			patterns::prop_pool = ptr.from_instruction().as<decltype(patterns::prop_pool)>();
			}, out);

		batch.Add({ XOR("PUP"), XOR("48 8B 05 ? ? ? ? 0F B7 50 10 48 8B 05") }, [](Ptr ptr) {
		...
		batch.Add({ XOR("SPBGH"), XOR("E8 ? ? ? ? E9 ? ? ? ? 3D ? ? ? ? 75 ? E8") }, [](Ptr ptr) {
			patterns::show_profile_by_gamer_handle = ptr.call().as<decltype(patterns::show_profile_by_gamer_handle)>();
			}, out);

		batch.Add({ XOR("N"), XOR("48 8B 0D ? ? ? ? 48 8B D7 E8 ? ? ? ? 84 C0 75 17 48 8B 0D ? ? ? ? 48 8B D7") }, [](Ptr ptr) {
		...
		batch.Add({ XOR("NS_BS"), XOR("48 B9 ? ? ? ? ? ? ? ? E9 ? ? ? ? FF C9 0F 84 ? ? ? ?  ") }, [](Ptr ptr) {
		...
		batch.Add({ XOR("TSE"), XOR("48 8B C4 48 89 58 08 44 89 48 20 55 56 57 48 83 EC 30 ") }, [](Ptr ptr) {
			patterns::trigger_script_event = ptr.as<decltype(patterns::trigger_script_event)>();
			}, out);
		...
		batch.Add({ XOR("VFX_WFX"), XOR("4C 8D 35 ?? ?? ?? ?? E9 ?? ?? ?? ?? 48 8B 45 20") }, [](Ptr ptr) {
		...
		batch.Add({ XOR("OQ"), XOR("74 41 4C 8B 05 ? ? ?") }, [](Ptr ptr) {
	...
	bool hooks() {
		...
		batch.Add({ XOR("IDKBRUH"), XOR("48 8B 05 ? ? ? ? 41 0F 94 C6 ") }, [](Ptr ptr) {
		...
		batch.Add({ XOR("RBB"), XOR(" 48 8B C4 48 89 58 ? 55 56 57 48 83 EC ? 48 83 60") }, [](Ptr ptr) {
			patterns::read_bitbuf_bool = ptr.as<decltype(patterns::read_bitbuf_bool)>();
			uint64_t target = ptr.as<uint64_t>();
